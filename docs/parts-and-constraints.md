...
|---|---|---|
| [Microchip ATmega328P datasheet, 7810D](https://ww1.microchip.com/downloads/en/devicedoc/atmel-7810-automotive-microcontrollers-atmega328p_datasheet.pdf) | SRAM, GPIO, pin-change interrupts, Timer1, TWI, ADC, DC characteristics | 2 KiB SRAM; 16 MHz CPU budget; hardware scheduling; explicit bus prescaler; logic-level limits. This is the manufacturer's automotive edition; confirm the actual chip grade separately |
...
| Experimental fast I2C burst | Reused 512-byte storage; 1,024 samples; linked port-read spacing verified at 16 cycles | START latency, 100 kHz transaction comparison, missed short states, finite 1.023 ms span |
...
I keep the continuous sniffer target at 10 kHz, below the PCF/host 100 kHz capability. Pin-change hardware coalesces edges arriving while interrupts are masked; an overflow flag cannot prove that all physical edges were seen. The separate experimental 1 MS/s burst mode records a short raw window for later decoding; it does not establish continuous 100 kHz support or guarantee 400 kHz capture. Fast-mode permits SCL highs shorter than its 1 µs sample interval. I use a reference instrument to qualify faster traffic. The 100 kS/s logic sampler also does not guarantee recognition of pulses shorter than a sample interval or correct reconstruction of signals close to its sampling rate.

The analog viewer uses A2 with AVCC as the reference, a 125 kHz ADC clock, and free-running 13-clock conversions after discarding the extended first conversion. I keep the source within 0–AVCC and about 10 kΩ or less in impedance for settling, consistent with the Microchip ADC sections above. Nominal spacing is 104 µs; accuracy also depends on AVCC, source impedance, noise and aliasing. There is no analog input conditioning on these probe pins.
